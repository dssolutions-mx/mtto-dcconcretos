-- funnel_config on checklist sections: mark generic checklist sections as Lane B (RH).
-- Default lane is maintenance (Lane A).

ALTER TABLE public.checklist_sections
  ADD COLUMN IF NOT EXISTS funnel_config jsonb;

COMMENT ON COLUMN public.checklist_sections.funnel_config IS
  'Funnel routing for checklist sections. Example: {"lane": "operations_evaluation"} for RH/ops items that must not create checklist_issues. Default (null) = maintenance.';

-- Backfill nulls are treated as maintenance in application code.
...
BEGIN
  ...
  LOOP
    ...
    v_funnel_lane := COALESCE(v_item->>'funnel_lane', 'maintenance');
    v_is_ops_eval := v_funnel_lane = 'operations_evaluation'
      OR v_section_type IN (
        'cleanliness_bonus',
        'security_talk',
