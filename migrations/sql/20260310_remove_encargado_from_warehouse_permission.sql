-- Remove ENCARGADO_MANTENIMIENTO from user_has_warehouse_permission function
-- No profiles use this role; COORDINADOR_MANTENIMIENTO is the current role.
-- Migration: 20260310_remove_encargado_from_warehouse_permission
...
BEGIN
  ...
  IF p_permission = 'release' THEN
    v_can_do := v_role IN ('GERENCIA_GENERAL','AREA_ADMINISTRATIVA','AUXILIAR_COMPRAS','ENCARGADO_ALMACEN','JEFE_UNIDAD_NEGOCIO','JEFE_PLANTA','COORDINADOR_MANTENIMIENTO','GERENTE_MANTENIMIENTO');
  ELSIF p_permission = 'receive' THEN
    v_can_do := v_role IN ('GERENCIA_GENERAL','AREA_ADMINISTRATIVA','AUXILIAR_COMPRAS','ENCARGADO_ALMACEN','JEFE_UNIDAD_NEGOCIO','JEFE_PLANTA','COORDINADOR_MANTENIMIENTO','DOSIFICADOR','GERENTE_MANTENIMIENTO');
  ELSIF p_permission = 'adjust' THEN
