...
export type IngresosGastosBody = {
  month: string // YYYY-MM format
  businessUnitId?: string | null
  plantId?: string | null
  /** When true, skip previous month + deltas (faster for KPI widgets that only sum current plants). */
  skipPreviousMonth?: boolean
  /**
   * When true, skip Cotizador financial/pumping/indirect queries and manual adjustments.
   * Use with skipPreviousMonth for dashboard diesel+maint KPI totals only (not full P&L rows).
   */
  costsOnly?: boolean
}

type SharedIngresosSnapshotData = {
  profilesData: { departamento: string | null; plant_id: string | null }[] | null
  dieselProducts: { id: string; price_per_liter: number | null }[] | null
}
...
async function calculateDieselCostsFIFO_IngresosGastos(
  ...
  const fifoLog = (...args: Parameters<typeof console.log>) => {
    if (reportsVerbose) console.log(...args)
  }
  const fifoWarn = (...args: Parameters<typeof console.warn>) => {
    if (reportsVerbose) console.warn(...args)
  }
  ...
  if (targetWarehouseIds.length === 0) {
    fifoWarn('[FIFO] No warehouses found for plant codes:', plantCodes)
    return new Map<string, number>()
  }

  fifoLog('[FIFO] plantCodes requested:', plantCodes, '| plants with warehouses:', [...new Set(warehouseToPlantCode.values())])
  ...
  fifoLog('[FIFO] Entries by plant:', Object.fromEntries(entriesByPlant))
  ...
  fifoLog('[FIFO DEBUG] Raw consumption data (unfiltered) - transfer breakdown:', Object.fromEntries(transferCounts))
  ...
  fifoLog('[FIFO] Consumptions per warehouse:', whSummary)
  ...
  const consumptionsInPeriod = allConsumptionsFetched.filter(cons => {
    if (isTransferFlag(cons.is_transfer)) {
      fifoWarn('[FIFO] Unexpected transfer row reached consumptionsInPeriod:', cons.id)
      return false
    }
    // Ensure we're using transaction_date, not created_at
    if (!cons.transaction_date) {
      fifoWarn('[FIFO] Warning: Consumption missing transaction_date:', cons.id)
      return false
    }
  ...
  fifoLog('[FIFO] Consumptions by plant (total fetched):', Object.fromEntries(consumptionsByPlant))
  ...
  fifoLog('[FIFO DEBUG] IN-PERIOD transfer check (from unfiltered):', Object.fromEntries(inPeriodTransferCheck))
  ...
  fifoLog('[FIFO] Consumptions by plant (in period - filtered):', Object.fromEntries(consumptionsInPeriodByPlant))
  ...
  fifoLog('[FIFO] Diesel liters by plant (in period, GMT-6, excl transfers):', JSON.stringify(literBreakdown, null, 0))
  fifoLog('[FIFO] Note: Gerencial report shows asset_only. Audit script raw=total.')
  ...
  allConsumptionsFetched.forEach(consumption => {
    ...
    if (isTransfer) {
      ...
      fifoWarn(`[Ingresos-Gastos FIFO] Skipping transfer consumption: ${consumption.id} (is_transfer=${consumption.is_transfer}, liters=${consumption.quantity_liters})`)
      return
    }
  ...
  transactionsByWarehouse.forEach((transactions, warehouseId) => {
          ...
          if (weightedAvgPrice > 0) {
            ...
            if (fallbackPrice > 0) {
              ...
              if (globalAvgPrice > 0) {
                ...
                consumptionCost += fallbackCost
                fifoWarn(`[FIFO] ${plantCode}: ${remainingLiters.toFixed(2)}L priced with global avg $${globalAvgPrice.toFixed(2)}/L`)
    ...
    if (plantCode === 'P004' || plantCode === 'P004P') {
      ...
      fifoLog(`[FIFO DEBUG ${plantCode}] Cost: $${totalConsumptionCost.toFixed(2)} | Liters: total=${lit?.total ?? 0}, asset-only=${lit?.asset_only ?? 0} (gerencial uses asset-only)`)
    }
  })
  
  // Debug: Log skipped transfers summary
  if (skippedTransfersCount > 0) {
    fifoLog(`[FIFO] Skipped ${skippedTransfersCount} transfer consumptions totaling ${skippedTransfersLiters.toFixed(2)}L from FIFO processing`)
  }
  ...
  consumptionCosts.forEach((cost, warehouseId) => {
    ...
    if (!plantCode) {
      fifoWarn(`[FIFO] Warehouse ${warehouseId} has no plant code mapping`)
      return
    }
...
export async function runIngresosGastosPost(params: {
  body: IngresosGastosBody
  supabase: any
  requestHost: string | null
  /** When true, always run full compute (e.g. rollup refresh cron). */
  bypassKpiRollupRead?: boolean
  /** Passed from API route auth for rollup read cache key (optional for scripts). */
  rollupReadUserKey?: string | null
}): Promise<Record<string, unknown>> {
  try {
    const { body, supabase, requestHost, bypassKpiRollupRead, rollupReadUserKey } = params
    const { month, businessUnitId, plantId, skipPreviousMonth, costsOnly } = body
    const costsOnlyMode = Boolean(costsOnly)
    ...
    const prevMonthStr = shiftMonthString(month, -1)
    const previousRange = getMonthRange(prevMonthStr)

    // Fetch business units and plants in parallel (rollup fast path and full compute both need both).
    ...
    const [{ data: businessUnits }, { data: plants }] = await Promise.all([
    ...
    if (!bypassKpiRollupRead && kpiRollupReadEnabled()) {
      const fromRollup = await attemptKpiRollupRead({
        month,
        comparisonMonth: Boolean(skipPreviousMonth) ? null : prevMonthStr,
        targetPlants,
        businessUnits: businessUnits || [],
        plants: plants || [],
        skipPreviousMonth: Boolean(skipPreviousMonth),
        costsOnly: costsOnlyMode,
    ...
    const cotizadorSupabase = costsOnlyMode
      ? null
      : createClient(
    ...
    const [{ data: profilesDataGlobal }, { data: dieselProductsGlobal }] = await Promise.all([
      costsOnlyMode
      ...
      supabase.from('diesel_products').select('id, price_per_liter'),
    ])

    const sharedSnapshotData: SharedIngresosSnapshotData = {
      profilesData: profilesDataGlobal,
      dieselProducts: dieselProductsGlobal,
    }
    ...
    const buildMonthSnapshot = async (range: ReturnType<typeof getMonthRange>) => {
      ...
      // Diesel + maintenance from gerencial report (in-process — no HTTP self-call)
      let gerencialData: Record<string, unknown> | null = null
      try {
        gerencialData = (await runGerencialReport(
          {
        ...
        console.error('[Ingresos Gastos Report] Gerencial in-process error:', err)
        gerencialData = null
      }

      // Calculate diesel costs using FIFO (primary method)
      const priceByProduct = new Map<string, number>()
      ;(sharedSnapshotData.dieselProducts || []).forEach(p =>
        priceByProduct.set(p.id, Number(p.price_per_liter || 0))
      )
      ...
      const gData = gerencialData as { plants?: unknown[]; assets?: unknown[] } | null
      const gerencialPlantMapByCode = new Map<string, any>()
      ;((gData?.plants || []) as any[]).forEach((plant: any) => {
      ...
      ;((gData?.assets || []) as any[]).forEach((asset: any) => {
      ...
      if (!costsOnlyMode) {
        ...
        ;(sharedSnapshotData.profilesData || []).forEach(profile => {
    ...
    const skipPrev = Boolean(skipPreviousMonth)

    let currentPlants: Awaited<ReturnType<typeof buildMonthSnapshot>>
    let previousPlants: Awaited<ReturnType<typeof buildMonthSnapshot>>

    if (skipPrev) {
      currentPlants = await buildMonthSnapshot(currentRange)
      previousPlants = []
    } else {
      ;[currentPlants, previousPlants] = await Promise.all([
        buildMonthSnapshot(currentRange),
        buildMonthSnapshot(previousRange),
      ])
    }
    ...
    return {
      month,
      comparisonMonth: skipPrev ? null : previousRange.periodMonth.slice(0, 7),
      plants: currentPlants,
      comparison: skipPrev
        ? { month: null, plants: [] as typeof currentPlants }
        : {
    ...
    console.error('Ingresos-Gastos compute error:', e)
    throw e
  }
}
