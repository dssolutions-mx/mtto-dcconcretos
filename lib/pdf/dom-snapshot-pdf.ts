...
/**
 * Rasterizes a DOM subtree to a multi-page A4 PDF (same pipeline as incidentes instantáneo).
 * Use `[data-pdf-chunk]` children for separate capture segments; otherwise one continuous document.
 */
export async function generateDomSnapshotPdf(
  element: HTMLElement,
  filenameBase = `documento-${format(new Date(), "yyyy-MM-dd-HHmm")}`,
): Promise<void> {
