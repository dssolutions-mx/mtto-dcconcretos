/**
 * Classify March 2026 Expenses
 *
 * This script reads INDIRECTOS_MARZO_INTELIGENTE.csv and classifies
...
// Mapping function to classify expenses (reused from December)
function classifyExpense(concepto: string, departamento: string): {
  ...
  if (conceptoUpper.includes('ADITIVO')) {
    return { categoria: '1', subcategoria: 'Químicos y Aditivos (Antidoping, Desincrustrante, Catalizadores)' }
  }
  ...
  if (conceptoUpper.includes('RENTA') && (conceptoUpper.includes('DEPARTAMENTO') || conceptoUpper.includes('CAMPAMENTO') || conceptoUpper.includes('CUARTO') || conceptoUpper.includes('POSADA') || conceptoUpper.includes('CASA') || conceptoUpper.includes('HOSPEDAJE') || conceptoUpper.includes('DEPTO'))) {
    return { categoria: '4', subcategoria: 'Renta de Inmuebles' }
  }

  // CAMPAMENTO P5 (sin prefijo RENTA; evita REPOSICION DE GASTOS DE CAMPAMENTOS)
  if (/^CAMPAMENTO\s+P\d/i.test(concepto.trim())) {
    return { categoria: '4', subcategoria: 'Renta de Inmuebles' }
  }
  ...
  // Category 5: SERVICIOS PROFESIONALES - Servicios Legales / Trámites
  if (conceptoUpper.includes('TRAMITES') || conceptoUpper.includes('TRÁMITES')) {
    return { categoria: '5', subcategoria: 'Servicios Legales y Trámites' }
  }

  if (conceptoUpper.includes('REGISTRO') || conceptoUpper.includes('PLANO') || conceptoUpper.includes('GRAVAMEN')) {
    return { categoria: '5', subcategoria: 'Servicios Legales y Trámites' }
  }
  ...
  if (conceptoUpper.includes('MANTO') ||
      ...
      conceptoUpper.includes('PIPA') ||
      (conceptoUpper.includes('ILERIA') &&
        (conceptoUpper.includes('ALB') || conceptoUpper.includes('LABA')))) {
  ...
  if (conceptoUpper.includes('GASTOS DE VENTA')) {
  ...
  if (conceptoUpper.includes('IMPUESTOS')) {
...
async function main() {
  console.log('🚀 Classifying March 2026 Expenses...\n')

  const csvPath = path.join(process.cwd(), 'INDIRECTOS_MARZO_INTELIGENTE.csv')
  const csvContent = fs.readFileSync(csvPath, 'utf-8')

  // Smart CSV parser - handles trailing empty columns and quoted CONCEPTO (incl. comas dentro del campo)
  function parseCSVLine(line: string): string[] {
    if (line.startsWith('"')) {
      ...
      const rest = line.slice(i).replace(/^\s*,\s*/, '')
      const remainingParts = rest.split(',').map(p => p.trim())
      const meaningful = remainingParts.filter(p => p !== '')
      if (meaningful.length >= 3) {
        return [concepto.trim(), meaningful[0], meaningful[1], meaningful[2]]
      }
    }

    const allCommas = line.split(',').map(p => p.trim())
    // Trim trailing empty columns (ENE26 tenía columnas vacías al final)
    let end = allCommas.length
    while (end > 4 && allCommas[end - 1] === '') end--
    const trimmed = allCommas.slice(0, Math.max(4, end))

    if (trimmed.length >= 4) {
      return [trimmed[0], trimmed[1], trimmed[2], trimmed[3]]
    }
    // CONCEPTO may contain commas - use last 3 as VALOR, PLANTA, DEPARTAMENTO
    if (allCommas.length > 4 && allCommas[3] && allCommas[4] === '') {
  ...
  const headers = ['CONCEPTO', 'VALOR', 'PLANTA', 'DEPARTAMENTO']
  ...
  const outputPath = path.join(process.cwd(), 'GASTOS_MARZO2026_CLASIFICADOS.csv')
  const outputLines = [
