/**
 * Classify January 2026 Expenses
 *
 * This script reads INDIRECTOS PROCESADOS ENE26.csv and classifies
...
function classifyExpense(concepto: string, departamento: string): {
  ...
  if (conceptoUpper.includes('RENTA') && (conceptoUpper.includes('DEPARTAMENTO') || conceptoUpper.includes('CAMPAMENTO') || conceptoUpper.includes('CUARTO') || conceptoUpper.includes('POSADA') || conceptoUpper.includes('CASA') || conceptoUpper.includes('HOSPEDAJE'))) {
...
async function main() {
  console.log('🚀 Classifying January 2026 Expenses...\n')

  const csvPath = path.join(process.cwd(), 'INDIRECTOS PROCESADOS ENE26.csv')
  const csvContent = fs.readFileSync(csvPath, 'utf-8')

  // Smart CSV parser - handles trailing empty columns and quoted CONCEPTO
  function parseCSVLine(line: string): string[] {
    // Quoted CONCEPTO field
    if (line.startsWith('"')) {
      ...
      if (closingQuoteIdx !== -1) {
        ...
        // Trim trailing empties - take first 3 non-empty for VALOR, PLANTA, DEPARTAMENTO
    ...
    // Trim trailing empty columns - January has CONCEPTO,VALOR,PLANTA,DEPARTAMENTO,,,
  ...
  const outputPath = path.join(process.cwd(), 'GASTOS_ENERO2026_CLASIFICADOS.csv')
  const outputLines = [
