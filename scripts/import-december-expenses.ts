/**
 * Import December 2025 Expenses from CSV
 * 
 * This script reads GASTOS_DICIEMBRE_CLASIFICADOS.csv and imports all expenses
 * as manual financial adjustments for December 2025.
 * 
 * Usage:
 *   npx tsx scripts/import-december-expenses.ts
 */
...
async function main() {
  console.log('🚀 Starting December 2025 Expense Import...\n')

  // Load CSV
  const csvPath = path.join(process.cwd(), 'GASTOS_DICIEMBRE_CLASIFICADOS.csv')
  const csvContent = fs.readFileSync(csvPath, 'utf-8')
  
  // Smart CSV parser - handles quoted fields and commas in CONCEPTO/SUBCATEGORIA_GASTO
  function parseCSVLine(line: string): string[] {
    const parts: string[] = []
    let i = 0
    
    // Helper to parse a field (handles quoted fields)
    ...
    try {
      if (row.TIPO_ASIGNACION === 'DIRECTO') {
        ...
        if (plantaOriginal === 'tj' || plantaOriginal === 'bj') {
          ...
          // Handle "tj y 3" format - extract BU and plant number
        ...
        // If specific plants are mentioned (like "tj y 3"), only use those
