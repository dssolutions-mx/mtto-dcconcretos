...
export function CorrectiveWorkOrderDialog({
  ...
  // Stable initialization - prevent unnecessary re-renders
  useEffect(() => {
    ...
    // Reset other states only when dialog opens
    setDescription("")
    ...
    // Only check for similar issues if online and not already checking
    if (isOnline && !checkingSimilar) {
      checkForSimilarIssues()
    }
  }, [open]) // ONLY depend on open state

  // Separate effect for handling online status changes
  useEffect(() => {
    if (open && isOnline && similarIssuesResults.length === 0 && !checkingSimilar) {
      checkForSimilarIssues()
    }
  }, [isOnline, open])
  ...
  const generateDefaultDescription = useCallback(() => {
    ...
    return desc.trim()
  }, [itemsWithIssues, checklist.name])

  const checkForSimilarIssues = useCallback(async () => {
  ...
  }, [checklist.assetId, itemsWithIssues])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const submissionData = {
        ...
        enable_smart_deduplication: isOnline,
        consolidation_window_days: 30,
      ...
      if (response.ok) {
        ...
        toast.error(`Error: ${errorData.error || errorData.message || 'Error al crear las órdenes de trabajo'}`)
      }
    } catch (error) {
  ...
  // Stable helper to update individual priority
  const updateIndividualPriority = useCallback((itemId: string, priority: string) => {
    if (process.env.NODE_ENV === 'development') {
  ...
  const getPriorityIcon = useCallback((priorityLevel: string) => {
  ...
  const getPriorityColor = useCallback((priorityLevel: string) => {
    switch (priorityLevel) {
      case "Alta":
  ...
  const getPriorityLabel = useCallback((priorityLevel: string) => {
    switch (priorityLevel) {
      case "Alta":
        return "Inmediata"
      case "Media":
  ...
  // Memoized priority configuration component to prevent unnecessary re-renders
  const PriorityConfiguration = React.memo(({
    priorityMode,
    setPriorityMode,
    globalPriority,
    setGlobalPriority,
    individualPriorities,
    updateIndividualPriority,
    itemsWithIssues,
    getPriorityIcon,
    getPriorityColor,
    getPriorityLabel
  }: {
    priorityMode: 'global' | 'individual'
    setPriorityMode: (mode: 'global' | 'individual') => void
    globalPriority: string
    setGlobalPriority: (priority: string) => void
    individualPriorities: Record<string, string>
    updateIndividualPriority: (itemId: string, priority: string) => void
    itemsWithIssues: any[]
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('🎨 Rendering PriorityConfiguration', { priorityMode, globalPriority, individualPriorities })
    }
    ...
    return (
      <div className="bg-gradient-to-r from-yellow-50 to-amber-50 p-4 rounded-lg border border-yellow-200">
        ...
        <RadioGroup 
          value={priorityMode} 
          onValueChange={(value: 'global' | 'individual') => {
            if (process.env.NODE_ENV === 'development') {
              console.log('🔄 Changing priority mode to:', value)
            }
              ...
              {itemsWithIssues.map((item, index) => (
                <div key={item.id} className="bg-gray-50 p-3 rounded-lg border">
                  ...
                  <RadioGroup 
                    ...
                    onValueChange={(value) => {
                      if (process.env.NODE_ENV === 'development') {
                        console.log(`⚡ Setting individual priority for ${item.id}: ${value}`)
                      }
                      updateIndividualPriority(item.id, value)
                    }}
                  >
  ...
  // Debug logs for monitoring (can be removed in production)
  if (process.env.NODE_ENV === 'development') {
    ...
    if (showResults && processingResults) {
      console.log('📊 Showing results dialog')
    } else {
      console.log('📝 Rendering main dialog content')
    }
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[95vw] sm:w-[90vw] md:w-[80vw] max-w-3xl h-[85vh] flex flex-col p-0">
        ...
        {showResults && processingResults ? (
          <div>Results are being shown</div>
        ) : (
          <>
              ...
              <div className="h-full overflow-y-auto px-4 sm:px-6">
                <div className="space-y-4 pb-4 pt-2">
                  ...
                  <PriorityConfiguration
                    ...
                    setGlobalPriority={setGlobalPriority}
                    individualPriorities={individualPriorities}
                    updateIndividualPriority={updateIndividualPriority}
                      ...
                      {(() => {
                        ...
                        return (
                          <div className="space-y-2">
                            ...
                            {/* Priority-specific information */}
                            {priorityMode === 'individual' && (
                              <div className="text-xs text-blue-600 mt-2 space-y-1">
                                <div>⚡ Prioridad individual asignada a cada problema</div>
                                {consolidateCount > 0 && (
                                  <div>🔄 Las prioridades más altas actualizarán automáticamente las órdenes existentes</div>
                                )}
                              </div>
                            )}
                            ...
                            {consolidateCount > 0 && (
                              <div className="text-xs text-blue-600 mt-1">
                                ...
                                {priorityMode === 'individual' && (
                                  <span className="block mt-1">🔥 Si la nueva prioridad es más alta, se escalará la orden existente</span>
                                )}
                              </div>
                            )}
                          </div>
